...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        for i in range(2*N):
            if i!=k:
                l_k_kN+=torch.exp(sim(z_k,out[i])/tau)
            if i!=k+N:
                l_kN_k+=torch.exp(sim(z_k_N,out[i])/tau)

        total_loss+=-torch.log(torch.exp(sim(z_k,z_k_N)/tau)/l_k_kN)-torch.log(torch.exp(sim(z_k_N,z_k)/tau)/l_kN_k)
        pass
...
def compute_sim_matrix(out):
    ...
    # TODO: Start of your code.                                                  #
    ##############################################################################
    sim_matrix=out.matmul(out.T)/(torch.matmul(torch.linalg.norm(out,dim=1,keepdim=True),torch.linalg.norm(out.T,dim=0,keepdim=True)))
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    mask = (torch.ones_like(exponential, device=device) - torch.eye(2 * N, device=device)).to(device).bool()#对角线为0，其余1
    ...
    denom = None
    denom=torch.sum(exponential,dim=1,keepdim=True)#分母
    # Step 2: Compute similarity between positive pairs.
    # You can do this in two ways: 
    # Option 1: Extract the corresponding indices from sim_matrix. 
    # Option 2: Use sim_positive_pairs().
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    positive_vec=sim_positive_pairs(out_left,out_right)#N x1
    pass
    ...
    numerator = None
    numerator=torch.exp(torch.cat((positive_vec,positive_vec),dim=0)/tau)#2N x 1，分子
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
